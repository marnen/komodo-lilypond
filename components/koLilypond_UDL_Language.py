# Komodo Lilypond language service.

import logging
from koUDLLanguageBase import KoUDLLanguage


log = logging.getLogger("koLilypondLanguage")
#log.setLevel(logging.DEBUG)


def registerLanguage(registry):
    log.debug("Registering language Lilypond")
    registry.registerLanguage(KoLilypondLanguage())


class KoLilypondLanguage(KoUDLLanguage):
    name = "Lilypond"
    lexresLangName = "Lilypond"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_clsid_ = "e6eef4be-5d66-4dbf-8036-961bd07781df"
    defaultExtension = '.ly'
